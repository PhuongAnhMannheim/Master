...
# Output:
moviereviewslog_path = '../Logs/movieReviews.log'
moviereviewsdb_path = '../Data/moviereviews.db'

# enable Logging:
logger = logging.getLogger()
fhandler = logging.FileHandler(filename=moviereviewslog_path, mode='a')
...
# + {"pycharm": {"name": "#%%\n"}}
conn = sqlite3.connect(moviereviewsdb_path)
c = conn.cursor()
c.execute("""
...
# + {"pycharm": {"name": "#%%\n"}}
merge_query = "UPDATE moviereviews "\
              "SET REVIEWRATING = (SELECT T.REVIEWRATING FROM moviereviews AS T "\
              "WHERE moviereviews.RATING = T.NODE AND moviereviews.URL = T.URL), "\
              "BESTRATING = (SELECT T.BESTRATING FROM moviereviews AS T "\
              "WHERE moviereviews.RATING = T.NODE AND moviereviews.URL = T.URL), "\
              "WORSTRATING = (SELECT T.WORSTRATING FROM moviereviews AS T "\
              "WHERE moviereviews.RATING = T.NODE AND moviereviews.URL = T.URL) "\
              "WHERE RATING IN (SELECT NODE FROM moviereviews);"
