...
def run(domain, trial, schema_rev_link, amazon_rev_link, schema_rev_size, amazon_rev_size):
    ...
    plt.savefig(f'results/test_series_chi_2/{title}.png', dpi=200)
    ...
    plt.savefig(f'results/test_series_chi_2/{title_norm}.png', dpi=200)
...
for d in domains:
    ...
    results = {}
    while limit >= 500:
        result = run(d, count_trial, schema_link, amazon_link, limit, 0)
        results[f"{d}_{count_trial}_{limit}_{0}"] = result
        result = run(d, count_trial, schema_link, amazon_link, 0, limit)
        results[f"{d}_{count_trial}_{0}_{limit}"] = result
        limit = limit - 100
    bigIndex = max([[results[key], key] for key in results])
    print(f'######## BEST RESULTS of trial{count_trial}_{d}')
    print(bigIndex)
